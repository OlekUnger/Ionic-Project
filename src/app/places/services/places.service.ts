import { HttpClient } from '@angular/common/http';
import { Injectable } from '@angular/core';
import { BehaviorSubject, Observable, of, Subscriber } from 'rxjs';
import { delay, map, switchMap, take, tap } from 'rxjs/operators';
import { AuthService } from '../../auth/services/auth.service';
import { BookingModel } from '../../bookings/models/booking.model';
import { IPlace, IPlaceResponseModel, PlaceModel } from '../models/place.model';

@Injectable({
    providedIn: 'root'
})
export class PlacesService {
    private static readonly url: string = 'https://ionic-angular-course-3479c.firebaseio.com';
    private static readonly offeredPlacesUrl = `${PlacesService.url}/offered-places.json`;

    private _places: BehaviorSubject<PlaceModel[]> = new BehaviorSubject<PlaceModel[]>([
        // new PlaceModel(
        //     'p1',
        //     'Manhattan Mansion',
        //     'In the heart of New York City.',
        //     'https://static.amazon.jobs/locations/58/thumbnails/NYC.jpg?1547618123',
        //     149.99,
        //     new Date('2019-01-01'),
        //     new Date('2019-12-31'),
        //     'abc'
        // ),
        // new PlaceModel(
        //     'p2',
        //     'L\'Amour Toujours',
        //     'A romantic place in Paris!',
        //     'https://images.adsttc.com/media/images/5d44/14fa/284d/d1fd/3a00/003d/newsletter/eiffel-tower-in-paris-151-medium.jpg?1564742900',
        //     189.99,
        //     new Date('2019-01-01'),
        //     new Date('2019-12-31'),
        //     'abc'
        // ),
        // new PlaceModel(
        //     'p3',
        //     'The Foggy Palace',
        //     'Not your average city trip!',
        //     'data:image/jpeg;base64,',
        //     99.99,
        //     new Date('2019-01-01'),
        //     new Date('2019-12-31'),
        //     'abc'
        // ),
    ]);

    get places(): Observable<PlaceModel[]> {
        return this._places.asObservable();
    }

    constructor(private _authService: AuthService, private _http: HttpClient) {
    }

    public fetchPlaces(): Observable<PlaceModel[]> {
        return this._http.get<{ [key: string]: IPlaceResponseModel }>(PlacesService.offeredPlacesUrl).pipe(
            map((response) => {
                const places = [];
                for (const key in response) {
                    if (response.hasOwnProperty(key)) {
                        places.push(new PlaceModel(
                            key, response[key].title,
                            response[key].description,
                            response[key].imgUrl,
                            response[key].price,
                            new Date(response[key].availableFrom),
                            new Date(response[key].availableTo),
                            response[key].userId
                        ));
                    }
                }
                return places;
            }),
            tap(places => {
                this._places.next(places);
            })
        );
    }

    public getPlace(id: string): Observable<PlaceModel> {
        return this._http.get<IPlaceResponseModel>(`${PlacesService.url}/offered-places/${id}.json`).pipe(
            map(response => {
                return new PlaceModel(
                    id,
                    response.title,
                    response.description,
                    response.imgUrl,
                    response.price,
                    new Date(response.availableFrom),
                    new Date(response.availableTo),
                    response.userId
                );
            })
        );
        // return this.places.pipe(
        //     take(1),
        //     map((places =>  {
        //         return {...places.find((p: PlaceModel) => p.id === id)};
        //     })
        // ));
    }

    public addPlace(place: IPlace): Observable<any> {
        const newPlace = new PlaceModel(
            Math.random().toString(),
            place.title,
            place.description,
            'https://images.adsttc.com/media/images/5d44/14fa/284d/d1fd/3a00/003d/newsletter/eiffel-tower-in-paris-151-medium.jpg?1564742900',
            place.price,
            new Date(place.dateFrom),
            place.dateTo,
            this._authService.userId
        );
        return this._http.post<{ name: string }>(PlacesService.offeredPlacesUrl, {...newPlace, id: null}).pipe(
            tap((response: { name: string }) => newPlace.id = response.name),
            switchMap(() => this.places),
            take(1),
            tap(places => {
                this._places.next(places.concat(newPlace));
            })
        );
    }

    public updatePlace(place: PlaceModel) {
        let updatedPlaces: PlaceModel[];
        return this.places.pipe(
            take(1),
            switchMap(places => {
                if (!places || !places.length) {
                    return this.fetchPlaces();
                } else {
                    return of(places);
                }
            }),
            switchMap(places => {
                const updatedPlaceIndex = places.findIndex(i => i.id === place.id);
                updatedPlaces = [...places];
                updatedPlaces[updatedPlaceIndex] = place;
                return this._http.put(`${PlacesService.url}/offered-places/${place.id}.json`, {
                    ...updatedPlaces[updatedPlaceIndex], id: null
                });
            }),
            tap(() => {
                this._places.next(updatedPlaces);
            })
        );
    }
}
